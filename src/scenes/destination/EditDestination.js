...
class EditDestination extends Component {
  ...
  componentDidMount() {
    this.props.apiRequest({
      url: `/destinations/${this.props.match.params.id}`,
      method: 'GET',
      accessToken: this.props.apiToken,
      onSuccess: (record) => this.setState({
        record,
        fileList: record.banners.map((item, index) => ({
          uid: `-${index + 1}`,
          name: item.image.match(/^.*?([^\\/.]*)[^\\/]*$/)[1],
          status: 'done',
          url: getImageURL(item.image, true, 86, 86)
        })),
        selectedUserId: record.ambassador_id
  ...
  onSave = () => {
    ...
    this.props.apiRequest({
      ...
      onSuccess: (payload) => this.setState({ spinning: false }, () => {
        message.success('The destination was updated successfully.');
      ...
      <div style={{ padding: 24, background: '#fff', minHeight: 360 }}>
        <Form>
          ...
          <Row>
            ...
            <Col sm={24} lg={12}>
              <Form.Item {...formItemLayout} label="Ambassador">
                {!isEmpty(this.state.ambassador) && this.renderAmbassador()}
              </Form.Item>
            </Col>
          </Row>
          <Row>
    ...
    <Row>
      ...
      >
        <div style={{ marginBottom: 8 }}>
          ...
          <Button type="primary" icon="plus" onClick={this.onAddVaccine}>Add</Button>
        </div>
        <Table
          columns={[{
            title: 'Name',
