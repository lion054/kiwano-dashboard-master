...
class EditStory extends Component {
  ...
  componentDidMount() {
    this.props.apiRequest({
      url: `/stories/${this.props.match.params.id}`,
      method: 'GET',
      accessToken: this.props.apiToken,
      onSuccess: (record) => {
        const fileList = record.items.map((item, index) => ({
          uid: `-${index + 1}`,
        ...
        this.setState({
          record,
          selectedUserId: record.author_id,
          fileList
        }, this.fetchAuthor);
      },
  ...
  onChangeUploadStatus = ({ file, fileList }) => {
    ...
    if (status === 'done') {
      ...
      fileList[fileList.length - 1].previewUrl = getImageURL(response.path);
